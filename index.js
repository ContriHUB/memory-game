...
let game_started = false; // Variable which tells if the game has started or not
function startGame() {
...
// If the orientation is in landscape, we simply start the game
if (document.documentElement.clientHeight <= document.documentElement.clientWidth)
{
    game_started = true;
    startGame();
}
else // Otherwise we keep listening for when the screen becomes landscape 
{
    window.addEventListener('resize', () => {
        if (!game_started) // We only need to check if the game hasn't started yet
        {
            setTimeout(() => {
                if (!game_started && document.documentElement.clientHeight <= document.documentElement.clientWidth)
                {
                    game_started = true;
                    startGame();
                }
            }, 200); // If the user was resizing the screen, then this delay gives them time to set final size
        }
    });
}